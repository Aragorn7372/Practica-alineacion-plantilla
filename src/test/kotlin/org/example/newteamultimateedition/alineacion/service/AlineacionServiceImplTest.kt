...
class AlineacionServiceImplTest {
    ...
    inner class ServiceGood {
        ...
        @Test
        @DisplayName("exportar data")
        fun exportarData() {
            val file = mock<File>()
            whenever(storage.fileWrite(alineacion, listOf(jugador), file)) doReturn (Ok((Unit)))
            val result = service.exportarData(alineacion, listOf(jugador), file)
            assertTrue(result.isOk)
        }
    }
    ...
    inner class AlineacionServiceTest {
        ...
        fun getByFechaSinEstar() {
            ...
            assertEquals(
                "Alineacion no encontrada con id: ${alineacion.juegoDate}",
        ...
        fun updateDatabaseException() {
            ...
            ) doThrow (RuntimeException("error en la base de datos"))
            val result = service.update(alineacion.id, alineacion)
            assertTrue(result.isErr, "deberia devolver Err")
            assertEquals(result.error.message, "error en la base de datos", "deberian ser iguales")
            verify(repository, times(1)).update(alineacion, alineacion.id)
        ...
        @Test
        @DisplayName("export data error")
        fun exportDataError() {
            val file = mock<File>()
            val error = AlineacionError.AlineacionInvalidoError("Error al reportar")
            whenever(storage.fileWrite(alineacion, listOf(jugador), file)) doReturn(Err(error))
            val result = service.exportarData(alineacion, listOf(jugador), file)
            assertTrue(result.isErr, "deberia devolver Err")
